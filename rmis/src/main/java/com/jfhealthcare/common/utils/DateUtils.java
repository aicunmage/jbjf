    ...
    public static List<Date> getCheckTime(Integer timeID){ 
    	...
    	switch(timeID){
	    	...
	    	case 1609://最近三年
                ...
                ds.add(getEndDateDay(new Date()));
                break;
            case 1610://值班时间  规定为17:00至次日8:00
                if (getDateHour(new Date()) < 17) {
                    // 当前时间为下午五点之前，则查询昨天下午五点到早上八点之间的数据
                    ds.add(stringToDate(formateDate(getSomeDaysBeforeAfter(new Date(), -1)) + " 17:00:00", patternD));
                    ds.add(stringToDate(formateDate(new Date()) + " 08:00:00", patternD));
                } else {
                    // 当前时间为下午五点以后，则查询下午五点到现在的数据
                    ds.add(stringToDate(formateDate(new Date()) + " 17:00:00", patternD));
                    ds.add(getEndDateDay(new Date()));
                }
                break;
	    	default:

...
import com.jfhealthcare.modules.teach.entity.TeachParentAndSon;
import lombok.Data;
...
public class ViewWorklistRequest extends BasicPageEntity{
    ...
    private String imageCenterId; //如果是影像中心管理员，此字段不为空
    
    private String loginCode;//将登陆账号传入
    
    private String ptnSource; // 患者来源 1--门诊；2--医院；3--体检

    private String timeOutTime; //超时时间设定

    private String timeOutWillBe; //即将超时时间设定

    private String rate; // 报告评级

    private String suggest; // 报告意见

    /**
     * 报告开始时间
     */
    @JsonFormat(pattern="yyyy-MM-dd HH:mm:ss")
    private Date checkRepStartTime;

    /**
     * 报告结束时间
     */
    @JsonFormat(pattern="yyyy-MM-dd HH:mm:ss")
    private Date checkRepEndTime;

    /**
     * 拒绝的教学图片目录ids
     */
    private List<TeachParentAndSon> checkPhotoIds;

    private Integer classifyId;//机构分类id

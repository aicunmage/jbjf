...
public class SysOperatorController {
    ...
    @AuthIgnore
    @ApiOperation(value = "IM获取用户信息", notes = "IM获取用户信息，这个接口用于IM")
    @RequestMapping(path = "/getUserForIM", method = RequestMethod.POST)
    public BaseResponse queryOperatorInfoById(@RequestBody Map idListInfo) {
        try {
            Map idListMap = (HashMap) idListInfo.get("content");
            List idList = (ArrayList <String>) idListMap.get("thirdUserIds");
            List <Map> operInfo = sysOperatorService.queryLoginInfoById(idList);
            return BaseResponse.getSuccessResponse(operInfo);
        } catch (Exception e) {
            log.error("查询用户信息失败!", e);
            return BaseResponse.getFailResponse("查询用户信息失败!");
        }

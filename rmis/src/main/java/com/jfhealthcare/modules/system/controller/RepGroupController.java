...
public class RepGroupController {
	...
	public BaseResponse queryRepGroup(@RequestBody RepGroupRequest repGroupRequest, @LoginUser LoginUserEntity loginUserEntity) {
		try {
			//查询当前用户下的分组
			String loginCode = loginUserEntity.getSysOperator().getLogincode();
			String imageCenterId = loginUserEntity.getSysOperatorDtl().getOrgId();
			if(imageCenterId != null && !"super".equals(loginCode)) {
				repGroupRequest.setImageCenterId(imageCenterId);
			}
			
			PageInfo<RepGroupResponse> queryRepGroup = repGroupService.queryRepGroup(repGroupRequest);
	...
	public BaseResponse saveRepGroup(@RequestBody RepGroupRequest repGroup, @LoginUser LoginUserEntity loginUserEntity) {
		try {
			...
			if (!"super".equals(loginUserEntity.getSysOperator().getLogincode())) {
				repGroup.setImageCenterId(loginUserEntity.getSysOperatorDtl().getOrgId());
			}
	...
	public BaseResponse updateRepGroup(@RequestBody RepGroupRequest repGroupRequest, @LoginUser LoginUserEntity loginUserEntity) {
		try {
			ValidatorUtils.validateEntity(repGroupRequest, Edit.class);
			TransferUtils.copyPropertiesIgnoreNull(repGroupRequest, repGroupRequest);
			redisUtils.delete(RedisEnum.REPGROUP.getValue()+":nancang");
			//增加所属影像中心
			if (!"super".equals(loginUserEntity.getSysOperator().getLogincode())) {
				repGroupRequest.setImageCenterId(loginUserEntity.getSysOperatorDtl().getOrgId());
			}
			repGroupService.updateRepGroup(repGroupRequest);
			return BaseResponse.getSuccessResponse();
		}catch (RmisException e) {
			log.error(e.getMessage(), e);
			return BaseResponse.getFailResponse(e.getMessage());
	...
	public BaseResponse queryRepGroupLogincodeById(@PathVariable String groupId) {
		try {
			...
			List<SysRepGroupResponse> srgs=repGroupService.queryLoginCodeByGroupId(groupId);
	...
	public BaseResponse queryRepGroupById(@PathVariable String groupId) {
		try {
			Assert.isBlank(groupId, "groupId不能为空!");
			RepGroupResponse repGroup=repGroupService.queryRepGroupByGroupId(groupId);

...
public class SysDictBodypartController {
	...
	public BaseResponse queryDictBodypartById(@PathVariable Integer id) {
		try {
	...
	public BaseResponse deleteDictodypart(@PathVariable Integer id) {
		try {
	...
	public BaseResponse updateDictBodypart(@RequestBody SysDictBodypartRequest sysDictBodypartRequest) {
		try {
			Assert.isNull(sysDictBodypartRequest.getId(),"id字段不能为空!");

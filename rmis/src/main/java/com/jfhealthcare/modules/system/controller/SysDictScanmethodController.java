...
public class SysDictScanmethodController {
	...
	public BaseResponse queryDictScanmethodById(@PathVariable Integer id) {
		try {
	...
	public BaseResponse deleteDictScanmethod(@PathVariable Integer id) {
		try {
			Assert.isNull(id, "id字段不能为空!");
	...
	public BaseResponse updateDictScanmethod(@RequestBody SysDictScanmethodRequest sysDictScanmethodRequest) {
		try {
			Assert.isNull(sysDictScanmethodRequest.getId(),"id字段不能为空");
